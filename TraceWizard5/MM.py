...
def format_MeterMaster4_header(pairs, **kwargs):
	...
	Submit keyword arguments customer_fields=[] and meter_fields=[] to ignore
	those sections.
	"""
	...
	if not header_volume_t_formatter:
		def header_volume_t_formatter(text):
			if '"' in text:
				text=text.replace('"','')
			if ',' in text:
				text=text.replace(',','')
			return TraceWizard4.volume_t(text)
	#
	if type(pairs) == dict:
		row = pairs
	else:
	...
	if not row:
		return None
	d = {}
	for key in kwargs.pop('customer_fields', 'CustomerID CustomerName Address City State PostalCode PhoneNumber Note'.split()):
		if key in row:
			d[key] = row[key].strip()
			del row[key]
...
#	# empty line in header would be here
	for key in kwargs.pop('meter_fields', 'Make Model Size Unit'.split()):
		if key in row:
			d[key] = row[key].strip()
			del row[key]
...
#	d['Unit']	= row['Unit'].strip()
	if 'StorageInterval' in row:
		try:
			d['StorageInterval'] = timedelta(seconds=float(row['StorageInterval']))
		except:
			warning("Bad storage interval string '{}'".format(row['StorageInterval']))
			d['StorageInterval'] = row['StorageInterval'].strip()
		finally:
			del row['StorageInterval']
	if 'NumberOfIntervals' in row:
		try:
			d['NumberOfIntervals'] = int(row['NumberOfIntervals'])
		except:
			warning("Bad number of intervals string '{}'".format(row['NumberOfIntervals']))
			d['NumberOfIntervals'] = row['NumberOfIntervals'].strip()
		finally:
			del row['NumberOfIntervals']
	if 'TotalTime' in row:
		...
		if m:
			...
			warning("Bad total time string '{}'".format(row['TotalTime']))
	...
	for key in 'BeginReading EndReading RegVolume MMVolume'.split():
		try:
			...
			warning("Bad {} string '{}'".format(key, row[key]))
			d[key] = row[key].strip()
		finally:
			del row[key]
	if 'ConvFactor' in row:
		"""
		...
		if ',' in cf:
			cf = cf.replace(",",".")
			info("Changed 'ConvFactor' = '{}' to '{}'".format(row['ConvFactor'], cf))
		try:
			d['ConvFactor'] = float(cf)
		except:
			warning("Bad conversion factor '{}'".format(row['ConvFactor']))
			d['ConvFactor'] = row['ConvFactor'].strip()
		finally:
			del row['ConvFactor']
	if row:
		info("{} (possibly empty) key(s) in header not parsed: {}".format(
			len(row), 
			', '.join("'{}'='{}'".format(k,v) for k,v in row.iteritems() ))
			)
	return d
